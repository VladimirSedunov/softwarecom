...
def test_parametrize(setup_browser, title, fio, email, phone, text, pers_data_agree, is_error, result_message):
    ...
    with allure.step("ТС5.3. Заполнить реквизиты формы"):
        ...
        # time.sleep(1)
        feedback_form.element('[type=submit][name=send]').with_(timeout=5.0).should(be.clickable).click()

    if not is_error:
        with allure.step(f"ТС5.4. Получить сообщение ({result_message})"):
            browser.element('.msg-wrap').with_(timeout=5.0).should(have.exact_text(result_message))
    else:
        with allure.step(f"ТС5.4. Получить сообщение ({result_message})"):
            feedback_form.element('.js-error').with_(timeout=5.0).should(have.exact_text(result_message))
    time.sleep(0.5)
